...
import { SearchResultItem, searchTvShowQuery } from '$/services/metadata';
...
export const TvShowSelection: FC<Props> = ({ value, hasError, onChange }) => {
  ...
  const { isLoading, data } = useQuery(
    searchTvShowQuery({ query: debouncedQuery, lang: 'de', enabled: debouncedQuery.length >= 3 })
  );
    ...
    <div className='flex flex-col gap-4'>
      <div className='flex flex-col gap-1'>
        <FormLabel className='flex items-center justify-between text-sm'>
          <span>{t('tvShowSelection.title')}</span>
        </FormLabel>

        <Input
          placeholder={t('tvShowSelection.placeholder')}
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className={cn(hasError && 'border-red-500')}
        />
        <FormDescription>{t('tvShowSelection.description')}</FormDescription>
      </div>
      <div className='h-[200px] overflow-y-auto pr-3'>
        {hasNoResults && <FormDescription>{t('tvShowSelection.noResults')}</FormDescription>}
